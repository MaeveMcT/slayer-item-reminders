...
The assignment page's `Monster variants` table is the best generic source because Wiki editors have already filtered it to useful assignment alternatives. The parser should extract **every** link from the first cell of each row, not classify bosses or maintain a local boss list. Most rows have one monster, but the Dagannoth Kings share one first cell containing three links; the current parser captures Prime but can omit Rex and Supreme.
